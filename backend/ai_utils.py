...
def preprocess_frames(frames):
    """
    Preprocesses a sequence of frames for AI inference.
    - frames: list of BGR numpy arrays
    - Returns: torch tensor of shape (16, 3, 224, 224)
    """
    processed_frames = []
    # Explicitly use float32 for normalization constants
    ...
    for frame in frames:
        ...
        # HWC to CHW
        chw = standardized.transpose(2, 0, 1)
        processed_frames.append(torch.from_numpy(chw))
    
    # Stack to (16, 3, 224, 224) and ensure float type
    return torch.stack(processed_frames).float()
